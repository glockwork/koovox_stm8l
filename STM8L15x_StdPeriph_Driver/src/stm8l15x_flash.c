...
/**
* @brief  Koovox_store_data_to_e2prom
* @param  
*		address:
*		offset: 
*		data:
...
* @retval none
*/
void Koovox_store_data_to_e2prom(uint32_t address, uint8_t offset, uint8_t* data, uint8_t size_data)
{
	...
	if(size_data > 1)
	{
		...
		for(; i<size_data; i++)
		{
			*(((PointerAttr uint8_t*)(uint16_t)address) + (i + offset)) = data[i];
		}
		
	}
	else
	{
		*(((PointerAttr uint8_t*)(uint16_t)address) + offset) = *data;
	}
...
/**
* @brief  Koovox_read_data_from_e2prom
* @param  
*		address:
*		offset: 
*		data:
...
* @retval none
*/
void Koovox_read_data_from_e2prom(uint32_t address, uint8_t offset, uint8_t* data, uint8_t size_data)
{
	FLASH_Unlock(FLASH_MemType_Data);
	...
	if(size_data > 1)
	{
		...
		for(; i<size_data; i++)
		{
			data[i] = *((PointerAttr uint8_t *)(uint16_t)address + (offset + i));
		}
	}
	else
	{
		*data = *((PointerAttr uint8_t *)(uint16_t)address + offset);
	}
